...
In particular, we test whether outflows may 
...
on the Balmer jump is modest. 
We demonstrate that a higher-density outflow model 
displays reasonable verisimilitude with the optical spectrum of RW Trianguli,
producing all the observed lines. In addition, this model
produces recombination continuum sufficient to fill in the Balmer jump,
although it also predicts stronger-than-observed 
...
We suggest that a nonhomogenous, `clumpy' wind may provide a possible
solution.
On the basis of these results, we suggest that winds can 
...
even near mid-eclipse \citep[e.g.][]{groot2004}. This points to line formation in a spatially
...
are too blue in the UV \citep{wade1988,long1991,long1994} and exhibit
...
been described extensively by LK02, SDL05 and H13, so here we provide
...
Monte Carlo estimator for the mean intensity in the line, as recorded during
...
light. $J_\nu$ is the Monte Carlo estimator for the mean intensity in the cell. 
...
found by LK02. The bottom panels in figure~\ref{wind} confirm that this
...
and IX Vel \citep[][see also figure 1]{beuermann1990}.
...
has an ionization parameter of $\log U \sim 2$ (see figure 6).
...
This system has exhibited P-Cygni profiles in He~\textsc{i}~$\lambda5876{\rm \AA}$
...
and the slight Helium photoabsorption edges present in the optical spectrum.
...
Figure~13 shows how the H$\alpha$
line changes with the kinematics of the wind
for an inclination of $80^\circ$. 
The line narrows as the models move towards denser,
more slowly accelerating winds. 
This is due the base of the wind becoming optically thick such 
that line emission cannot escape. This means
what is effectively a photosphere for the line moves 
to a greater vertical distance from the disk plane,
where rotational velocities drop off as $1/r$. 
The narrowing of the line is {\em not} due to the radial velocity shear
mechanism proposed by MC96.
The main criterion for this mechanism to work is that

\begin{equation}
\frac{dv_p}{dr} \gtrsim \frac{dv_\phi}{dr},
\end{equation}

where $v_p$ is the poloidal velocity and $v_\phi$
is the Keplerian velocity. This criterion is simply 
not satisfied in the emission region of our models.
MC96 find values of $2-6.5$ for the ratio $(dv_p/dr)/(dv_\phi/dr)$
in their simple model, but in even our fastest accelerating 
wind we find ratios of $\lesssim0.1$ in the main \ha\ emission region.
The relatively low poloidal gradients are required to produce high densities
and observable emission lines, although 
we stress that the velocity field may depend significantly 
on the particular parameterisation used. 
We also suggest investigating clumping in CV winds.
Inhomogeneity could allow higher density pockets 
of emission to exist while still possessing a high radial velocity shear.
Theory of line-driven winds predicts an unstable flow
\citep{macgregor1979,owockirybicki1984,owockirybicki1985},
and simulations of accretion disk winds find a nonhomogeneous 
density structure \citep{proga1998,pkdh2002}.
In addition, a number of implementations have been used in the stellar wind
community \citep[e.g.][]{hamannkoesterke1998,oskinova2007}. 


The emission in our models which produce single-peaked lines comes from
approximately $10^{11}$~cm ($\sim150~R_{WD}$) above the disk plane.
Most models for Balmer emission in CVs propose that the Balmer emission
is more or less confined to the disk plane (e.g, MC96; REFs), 
and one might naively expect that the size of the emission region
is comparable to the size of the accretion disk, i.e., 10s-100s of WD radii.
However, the true vertical extent of the emission region is not known,
and it has been shown in multiple observations of eclipsing NLs that the 
H$\alpha$ line is only moderately eclipsed compared to the continuum 
(REFs, see also section 5.4).
We do not necessarily proposed that the emitting region is this large,
but it is a natural avenue to explore and a simple way in which to form
single-peaked lines.

% Alternatively, there are possible ways in which the model
% could be altered to better reproduce single-peaked
% lines as a result of radial velocity shear. First,
% one could adopt a more equatorial wind geometry. 
% This would increase the velocity gradient in the radial 
% direction. However, CV winds are known to be moderately collimated.
% This is demonstrated by the fact that blue-shifted resonance line 
% absorption is seen in low inclination systems such as RW Sex \citep{greensteinoke1982},
% but not in eclipsing systems \citep{cordovamason1985}.
% Furthermore, the main reason for adopting this type of 
% wind geometry is that it already successfully models the 
% P-Cygni resonance lines seen in the UV (LK02; Noebauer et al. 2010),
% Equatorial winds are expected in AGN and X-ray binaries 
% \citep{weymann1991, ponti2012}, so these winds may be more likely
% to possess the required kinematics.
% Second, one could adopt velocity laws with higher 
% velocity gradients in the line emission region. 
% However, this poses problems for a smooth biconical wind. 
% We have already demonstrated that high densities are required 
% to produce significant emission. In a smooth model, which
% satisfies the mass continuity condition (see section 3.1), 
% it is therefore difficult to reconcile 
% these high densities with the high velocity gradients required.

% We suggest that clumping
% in CV winds provides a possible solution.
% Inhomogeneity could allow higher density pockets 
% of emission to exist while still possessing a high radial velocity shear.
% Theory of line-driven winds predicts an unstable flow
% \citep{macgregor1979,owockirybicki1984,owockirybicki1985},
...
% In addition, a number of implementations have been used in the stellar wind
% community \citep[e.g.][]{hamannkoesterke1998,oskinova2007}. 
% We therefore hope that further investigation of
% clumpy winds could help shed light on the formation of single-peaked 
% line emission in high inclination systems.
% It should be stressed again that these conclusions may, as mentioned in 
% section 5.1, be dependent on our particular parameterisation.
% By way of example, specific assumptions about the initial and 
% terminal velocities may significantly affect velocity gradients
% in regions dominated by rotational velocities.
...
We observe a narrowing of the lines
but this is not due to radial velocity shear (see section 5.3
for a discussion).
}
\label{halpha}
\end{figure} %fullpage
...
particularly sensitive to this value. We emphasize that this
...
although the double peaked lines narrow 
...
We suggest that nonhomogenous, `clumpy' line-driven winds may offer a 
potential solution.
...
We can also make a number of basic,
