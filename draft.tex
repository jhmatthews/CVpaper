...
We suggest that a non-homogenous, `clumpy' wind may provide a possible
solution.
...
However, it leads to absorbing columns in \ha\ of $N_H\sim ??$ - far too
low to produce noticeable blue shifted absorption. 
While we suspect that the winds that exhibit such profiles must 
lie in a different ionization regime, requiring 
significantly different disk SEDs or mass-loss rates to a 
`standard' high-state CV. However, we note it is difficult for this
idea to exist within a line-driven wind framework.
...
discussed further in section~\ref{modelb}, the 
...
This is due
the base of the wind becoming optically thick such 
...
The main criterion for this mechanism is

\begin{equation}
\frac{dv_p}{dr} > \frac{dv_\phi}{ds}\,
\end{equation}

where $v_p$ is the poloidal velocity and $v_\phi$
is the Keplerian velocity.
This criterion is simply not satisfied in the emission region of our models. 

There are two possible ways in which the model
...
We note that equatorial winds are expected in AGN and X-ray binaries 
\citep{ponti2012}, and that these winds may be more likely
to exhibit the required kinematics.
...
Non-homogeneity could allow higher density pockets 
of emission to exist with radial velocity shear still being high.
Line-driven winds are expected to be unstable (REFS),
and a number of implementations have been used in the stellar wind
community (REFS). We therefore hope that further investigation of
clumped winds could help solve the current `mystery' of single-peaked 
line emission in high inclination systems.



% Although \ha\ is single-peaked in our model, the majority of the
% lines are double-peaked, particularly  
% Figure~\ref{halpha} shows how the H$\alpha$
% % He~{\sc ii}~$\lambda 4686$ 
% line changes with the kinematics of the wind
% for an inclination of $80^\circ$. 
% % The width and strength of He~{\sc ii} 
...
% line becomes optically thick. This reproduces the behaviour predicted by fsingle-pe96, who
...
but this is not due to the radial velocity shear mechanism (see section 5.3
for a discussion).
...
narrow. This is in contrast to what is generally seen in observations
of NLs, although the relatively low resolution of the RW Tri
spectra makes a specific comparison difficult.
...
%  Here we could discuss single-peaked emission and trends with parameters.
% \label{modelb}
...
% properties of the outflow. In particular, there are three key questions
% we would like to address: (i) can a modified wind model account 
% for all of the lines we see in optical spectra of CVs while preserving
% the UV behaviour;
% (ii) under what conditions can disk winds
% produce {\em single-peaked} Balmer emission lines (MC96); 
% (ii) can the wind-formed recombination continuum completely
...
% Figure~\ref{halpha} shows how overall spectrum
% changes with the kinematics of the wind for an inclination
...
% \begin{figure} %fullpage
% \mbox{
% % \subfigure{\includegraphics[width=0.33\textwidth]{figures/3by3_grid_lyman.eps}}  
% % \quad
...
although single-peaked emission can be formed in our densest models, 
...
approximately $10^{11}$~cm above the disk plane, which is not supported
by eclipse-mapping observations (REFS). We suggest
that non-homogenous, 
`clumpy' line-driven winds may offer a potential solution.
